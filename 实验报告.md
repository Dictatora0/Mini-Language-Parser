...
从编程实现角度，可以把语法分析的总体流程抽象为如下伪代码（简化自 `parse_to_ast` 与 `ASTParser.parse`）：

```text
function parse_to_ast(source_code, enable_semantic_check=True):
    tokens = Lexer(source_code).tokenize()
    if 存在 token.type == ERROR:
        收集所有词法错误信息并返回 (None, errors, None)

    parser = ASTParser(tokens, source_code)
    ast = parser.parse()          # 从 <program> 开始递归下降

    if parser.errors 非空:
        return (ast, parser.errors, parser.symbol_table)

    if enable_semantic_check 且 ast 非空:
        semantic_errors = analyze_semantics(ast, parser.symbol_table)
        if semantic_errors 非空:
            return (ast, parser.errors + semantic_errors, parser.symbol_table)

    return (ast, [], parser.symbol_table)

class ASTParser:
    function parse():
        root = self.program()     # 对应 <program> 产生式
        if 当前 token 不是 EOF:
            self.error("程序结束后有多余的内容")
        if self.success 且 self.errors 为空:
            return root
        else:
            return None
```
...
为了更直观地说明各个阶段之间的衔接，本节以演示脚本中的阶乘程序为例，串联 AST 生成、语义分析和解释执行的全过程。示例程序如下：
...
在这个阶乘示例中，首先由示例代码调用 `parse_to_ast(source_code)`，内部依次执行词法分析和语法分析，最终得到一棵以 `Program` 为根节点的抽象语法树。根节点下面的 `var_declarations` 记录了 `n: integer` 和 `fact: integer` 两个变量声明，`block` 节点则包含对这两个变量的初始化语句以及一个 `WhileStatement` 循环语句，其循环条件为关系表达式 `n > 0`，循环体内部依次对 `fact` 进行乘法累积并将 `n` 递减直至条件不再满足。借助 `print_ast(ast)` 可以直观地看到这一树形结构及其嵌套关系。
...
| 规则类别            | 语法构造                         | 操作数/变量类型要求                             | 结果类型                    | 违反时错误形式（示例）                             |
| ------------------- | -------------------------------- | ----------------------------------------------- | --------------------------- | -------------------------------------------------- |
| 基本字面量          | 数字、字符串、布尔字面量         | 数字区分整数/实数；字符串、布尔按字面确定类型   | INTEGER/REAL/STRING/BOOLEAN | `变量 'x' 未声明`（变量引用缺失）                  |
| 赋值语句            | `x := expr`                      | `expr` 类型可通过 `can_convert` 转为 `x` 的类型 | 与左侧变量相同              | `类型不匹配: 不能将 … 赋值给 … 类型的变量 'x'`     |
| 算术运算            | `a + b`、`a - b` 等              | `a`、`b` 均须为 INTEGER 或 REAL                 | INTEGER 或 REAL             | `算术运算 '+' 的左/右操作数必须是数值类型`         |
| 关系运算            | `a < b`、`a = b` 等              | `a`、`b` 均须为 INTEGER 或 REAL                 | BOOLEAN                     | `关系运算 '<' 的左/右操作数必须是数值类型`         |
| 逻辑运算（二元）    | `p and q`、`p or q`              | `p`、`q` 均须为 BOOLEAN                         | BOOLEAN                     | `逻辑运算 'and' 的左/右操作数必须是 boolean 类型`  |
| 逻辑运算（一元）    | `not p`                          | `p` 须为 BOOLEAN                                | BOOLEAN                     | `逻辑非的操作数必须是 boolean 类型`                |
| if/while 条件表达式 | `if cond then` / `while cond do` | `cond` 推导类型须为 BOOLEAN                     | BOOLEAN                     | `if/while 语句的条件必须是 boolean 类型，但得到 …` |
| 输入/输出           | `read(x)` / `write(expr)`        | `read` 中变量需事先声明；`write` 表达式类型任意 | 与变量/表达式一致           | `read 语句中的变量 'x' 未声明` 等                  |
...
解释器在执行阶段增加了多处运行时保护，用于防止无限循环、异常数值和不合理的输入输出导致程序失控或崩溃。表 3 总结了主要的保护点及其触发条件。

表 3 解释器运行时保护机制汇总

| 保护点               | 触发条件                                               | 对应实现位置                       | 行为/错误信息示例                                           |
| -------------------- | ------------------------------------------------------ | ---------------------------------- | ----------------------------------------------------------- |
| 全局循环次数上限     | 所有 `while` 总迭代次数超过 `MAX_LOOP_ITERATIONS`      | `Interpreter.visit_WhileStatement` | 抛出 `RuntimeError("程序执行超过最大循环次数限制…")`        |
| 单个循环局部计数提示 | 单个 `while` 循环局部计数超过 1000 次                  | 同上                               | 通过 `log` 输出警告，不中断程序                             |
| 输出行数上限         | `write` 调用累计输出行数超过 `MAX_OUTPUT_LINES`        | `Interpreter.visit_WriteStatement` | 抛出 `RuntimeError("输出行数超过限制…")`                    |
| 除零与无效除法结果   | 二元运算 `/` 右操作数为 0 或结果为 NaN/Inf             | `Interpreter.visit_BinaryOp`       | 抛出 `RuntimeError("除零错误"/"除法产生无效结果")`          |
| 非数值算术运算       | `+ - * /` 的任一操作数不是 int/float                   | 同上                               | 抛出 `RuntimeError("算术运算的左/右操作数必须是数值类型…")` |
| 未定义变量读取       | 在 `visit_Variable` 中访问未在 `global_scope` 中的变量 | `Interpreter.visit_Variable`       | 抛出 `RuntimeError("变量 'x' 未定义")`                      |
| 输入格式不匹配       | `read` 时用户输入无法按期望类型转换                    | `Interpreter.visit_ReadStatement`  | 抛出 `RuntimeError("输入格式错误: 期望整数/实数…")`         |

这些运行时保护与前面的语义检查互为补充：语义分析阶段尽可能在编译时发现类型问题，而解释器在运行时对循环、输出和数值运算设置安全边界，从而在教学示例和不可信输入场景下都能保持较好的稳定性。
...
当前工程中，针对 Mini 语言编译器实现了两套互补的自动化测试，用于系统性验证语法分析器的行为：

- 综合场景测试脚本 `tests/test_parser_comprehensive.py`：

  - 按功能划分为 6 大类测试：赋值语句、if-then 结构、while 循环、块结构、不同级别的表达式、错误处理。
  - 每一类内部包含多条用例（当前共 57 条），既包括合法程序，也包括带有典型语法错误的程序。
  - 每个用例直接构造 Mini 源程序字符串，调用 `parse_from_source()`，根据返回字符串是否包含成功提示或特定错误关键字来判断是否通过。

- 表驱动测试脚本 `tests/test_cases.py`：
  - 使用一个大的 `TEST_CASES` 字典存放测试用例，按类别组织，例如“表达式解析测试”“语句结构测试”等。
  - 每个用例记录名称、源代码和期望结果（完整提示或关键字），`run_all_tests()` 统一遍历并统计通过情况。
  - 在原有用例基础上，当前版本补充了与新实现直接相关的测试类别：
    - 变量声明与类型测试：覆盖多变量、多行声明、重复声明、缺少类型关键字等场景；
    - 输入输出语句测试：覆盖 `write(表达式)` 与 `read(变量)` 的正常形式，以及缺少括号、缺少表达式或变量名、缺少右括号等错误；
    - 词法错误测试：覆盖非法字符、未闭合字符串等，由 `Lexer` 生成 `TokenType.ERROR`，在 `parse_to_ast()` 中统一转换为“词法错误”提示。

综合来看，两套测试从不同维度覆盖了解析器：综合场景测试更接近“按功能模块分组的演示用例”，表驱动测试则便于批量回归和后续扩展。
...
1. 直接运行综合测试脚本：
   ...
   脚本内部会按测试类别依次执行所有用例，并在末尾输出“测试总结: 通过数量/总数量”。
...
2. 运行表驱动测试脚本：
   ...
   该脚本会遍历 `TEST_CASES` 中的所有类别和用例，打印每个用例的名称、代码片段和实际结果，最后给出整体通过情况。
...
3. 通过统一入口在交互环境中调用：
   ...
   `tests/__init__.py` 中定义的 `run_all_tests()` 会依次调用综合测试和表驱动测试的入口函数，仅当两套测试全部通过时返回 `True`。

在 `main.py` 中，`run_tests()` 函数当前仍调用 `tests.test_cases.run_all_tests()`，适合作为语法分析核心功能的基本回归测试；若希望同时运行综合测试，可以在命令行或交互式环境中显式从 `tests` 包导入并调用统一入口。

### 5.3 测试结果

在当前代码版本下，通过 `python -c "from tests import run_all_tests; print(run_all_tests())"` 可以看到，两套测试脚本中的全部用例均能通过，最终返回值为 `True`。在综合测试脚本中，输出的“测试总结”也表明 57 个综合用例全部通过；在表驱动测试脚本中，各类别用例同样全部达到预期结果。

从实现角度看，测试结果还反映出 `parse_from_source()` 的职责已经被重新划分为“只做语法检查”：它内部调用 `parse_to_ast(source_code, enable_semantic_check=False)`，仅在语法分析阶段发现错误时返回带有行列信息的语法错误字符串；变量是否声明、类型是否匹配等问题统一由语义分析阶段处理，不再混入语法分析错误之中。这一点与测试脚本中对错误关键字的检查完全一致，也符合编译前端“词法—语法—语义”分层设计的原则。
